...
#
# Script generated by Vivado on Thu Dec 19 16:29:46 +0800 2024
...
#
# Usage        : bclk.sh
#                bclk.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                bclk.sh [-reset_run]
#                bclk.sh [-reset_log]
#                bclk.sh [-help]
#
...
#                script (type 'bclk.sh -help' for more information).
#
...
# script info
echo -e "bclk.sh - Script generated by export_simulation (Vivado v2023.1 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./bclk.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
compile()
{
  vlogan -work xpm $vlogan_opts -sverilog +incdir+"../../../../core.srcs/sources_1/ip/bclk" \
...
elaborate()
{
  vcs $vcs_elab_opts xil_defaultlib.bclk xil_defaultlib.glbl -o bclk_simv
}

# RUN_STEP: <simulate>
simulate()
{
  ./bclk_simv $vcs_sim_opts -do simulate.do
}
...
reset_run()
{
  files_to_remove=(ucli.key bclk_simv vlogan.log vhdlan.log compile.log elaborate.log simulate.log .tmp_log .vlogansetup.env .vlogansetup.args .vcs_lib_lock scirocco_command.log lib_sc.so 64 AN.DB csrc bclk_simv.daidir vcs_lib c.obj)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./bclk.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: bclk.sh [-help]\n\
Usage: bclk.sh [-step]\n\
Usage: bclk.sh [-lib_map_path]\n\
Usage: bclk.sh [-reset_run]\n\
Usage: bclk.sh [-reset_log]\n\
Usage: bclk.sh [-keep_index]\n\
Usage: bclk.sh [-noclean_files]\n\n\
